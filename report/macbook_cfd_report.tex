...
\twocolumn[%
  ...
  air---agree with the range reported by five independent sources to
  within \SI{1.2}{\celsius}.
...
$\alpha_{u},\;\alpha_{p}$      & Momentum / pressure relaxation (0.7 / 0.8) \\
...
($\approx$\,2\,\si{\percent} of channel height). The eddy viscosity
...
$n \approx \num{50e3}$ (3D) flow-cell unknowns, the fill-in remains
...
($k = \SI{2000}{W\per\meter\per\kelvin}$, copper) and low-$k$ air
...
six face neighbours are all inside the flow mask---which measures
...
\begin{table}[!tbp]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    Total cells                   & \num{46800}            & \num{89280} \\
    Flow cells                    & \num{18875}            & \num{49662} \\
    Runtime (laptop)              & $\sim\!\SI{6}{s}$      & $\sim\!\SI{20}{s}$ \\
    Iterations                    & \num{1997}             & \num{867} \\
...
Twenty-two components model the SoC (\SI{15}{\watt}), dual RAM
modules (\SI{2}{\watt} each), dual fan blowers, vapor chamber
($k = \SI{2000}{W\per\meter\per\kelvin}$), logic board, SSD
controllers, speakers, and six battery cells. The 3D model adds
a thermal-interface-material (TIM) layer above the SoC die with
$\kTIM = \SI{0.14}{W\per\meter\per\kelvin}$ in a \SI{1}{mm}-thick
slab. This reproduces a die-to-vapor-chamber resistance of
$R_{jc} \approx \SI{3}{K\per\watt}$, consistent with published
silicon junction-to-case values.
...
cross-section at $y = \SI{124.5}{mm}$. The vertical cut shows air
entering the narrow bottom-case slit at $z = 0$, rising through
the battery cavity, deflecting around the logic board, accelerating
through the fan columns, and exiting laterally through the rear
hinge outlets.
...
evolution. The interior divergence sits at machine precision
($\sim\!\num{e-13}$\,\si{\per\second} in 2D;
$\sim\!\num{e-11}$\,\si{\per\second} in 3D) from iteration one.
...
\begin{figure*}[!tbp]
  ...
  \caption{2D grid refinement at three resolutions ($130\!\times\!90$,
    ...
    spacing $1/n_{x}$. Green bands: published measurement ranges.
...
Lateral metrics are mesh-converged. The SoC is mesh-sensitive
because different grids quantize the SoC bounding box into
different effective surface-area-to-volume ratios. The SoC
prediction nonetheless remains inside the published
82--98\,\si{\celsius} band at every resolution.
...
the SoC die and the vapor chamber. This reproduces
$R_{jc} \approx \SI{3}{K\per\watt}$. The 2D model has no explicit
...
\SI{34}{\celsius}, and exhaust \SI{40}{\celsius}. All eight values
fall within the 82--98\,\si{\celsius} (SoC),
35--41\,\si{\celsius} (battery), 27--37\,\si{\celsius} (palm rest),
and 40--50\,\si{\celsius} (exhaust) published bands.
